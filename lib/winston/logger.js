...
class Logger extends Transform {
  ...
  log(level, msg, ...splat) {
    ...
    if (arguments.length === 1) {
      ...
      const info = level instanceof Error ? this._cloneError(level) : { ...level };
      Object.assign(info,{ [LEVEL]:level.level },this.defaultMeta);
      this.write(info);
      return this;
    }
    ...
    if (arguments.length === 2) {
      if (msg && typeof msg === 'object') {
        const info = msg instanceof Error ? this._cloneError(msg) : { ...msg };
        Object.assign(info,{ level,[LEVEL]:level },this.defaultMeta);
        this.write(info);
        return this;
      }
  ...
  /**
   * Utility function that clones given error object. Returns the new error object.
   * @param {Error} error
   * @returns {Error}
   * @private
   */
  _cloneError (error) {
    ...
    Object.assign(errorClone,error);
    return errorClone;
  }
